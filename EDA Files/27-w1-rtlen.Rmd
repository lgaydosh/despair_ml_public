...
The purpose of this document is to perform EDA on the Wave 1 road length variables (1 used).

These variables are: \ 

'w1rtarl1' : Length of roads (m), all CFCC road types, 1 km \
...
#contextual data
context_file <- str_c("Z:/Gaydosh/ONE - Obesity and Neighborhood Environment Files/w1_ONE/Wave I Road Type Length Data/w1rtlen.xpt")
...
final_data_all <- data_combined_all %>%
  dplyr::select(aid, tidyselect::all_of(w1rl_list))
```
...
```{r}
#only 1 variable, no correlation heatmap needed
...
```
```{r correlation plot among variables with high missingness, eval=FALSE, cache=TRUE, fig.width=25,fig.height=25}
...
#q1_w1rl <- hist_by_var(final_data_all, c('bst90592', 'bst90593',
...
```{r correlation funnel for variables most of interest, warning=FALSE, cache=TRUE}
 ...
 w1rl_and_outcomes <- c(w1rl_list,
 "h5ss0b", "h4pe6", "h4pe7") #depression, anxiety, optimism

final_data_corr <- data_combined_all %>%
  dplyr::select(aid, tidyselect::all_of(w1rl_and_outcomes))
