...
The purpose of this document is to perform EDA on the prosocial behavior variables (8 used).
The prosocial behavior variables are: \ 


  **Wave 3**:\
    1. `h3cc6`: organ donor \
    2. `h3cc7`: voter registration \
    3. `h3cc8`: voting behavior \
    4. `h3cc3`: volunteer work \
    \   
  **Wave 4**:\
    1. `h4da27`: voting behavior\
    2. `h4da26`: volunteer work \
    \
  **Wave 5**:\
    1. `h5ss8`: voting behavior  \
    2. `h5ss7`: volunteer work \
    \
...
# prosocial behavior variables as listed above
ps_list <- c(
              'h3cc6', 'h3cc7', 'h3cc8', 'h3cc3', #wave 3
              'h4da27', 'h4da26', #wave 4
              'h5ss8', 'h5ss7') # Wave 5
...
final_data_all <- data_combined_all %>%
  dplyr::select(aid, tidyselect::all_of(ps_list))
```
...
From `plot_missing()` we see that wave 3 has most missingness followed by 4, 5, and 1.
...
From the barplot we see that about 2500 people have 4 variables missing (wave 3 has 4 variables) and about 1000 have 2 variables missing (wave 5 and 4 each has 2 variables). Besides, about 9000 have no missing values which means that they participated in all the wave1, wave3, wave4, and wave5. Using `drop_na()` will result in too much missing information without explanation for why those people are missing those variables.
...
#these questions were asked in the same way across waves
q1_ps <- hist_by_var(final_data_all, c('h4da27', 'h5ss8'), 5, "voting frequency level")
q2_ps <- hist_by_var(final_data_all, c('h4da26', 'h5ss7'),7 ,"volunteer hours in last year (categories)")

grid.arrange(q1_ps,q2_ps, nrow = 1)
 
```



Here is the visualization of how answers change for the same question over different waves. Voting distribution appears to change with higher percentage of frequent voters. Volunteer hours distribution appears consistent and skewed right.
...
ps_and_outcomes <- c(ps_list, "h5ss0b", "h4pe6", "h4pe7") #depression, anxiety, optimism

final_data_corr <- data_combined_all %>%
  dplyr::select(aid, tidyselect::all_of(ps_and_outcomes))
