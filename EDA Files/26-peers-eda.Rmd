...
The purpose of this document is to perform EDA on the peer and family behavior variables (12 used).
The peer and family behavior variables are: \ 

  **Wave 1**:\
    1. `h1su4`: Friend suicide attempt \
    2. `h1su6`: Family suicide attempt \
    3. `h1su5`: Friend suicide completion \
    4. `h1su7`: Family suicide completion \
    \
  **Wave 3**:\
    1. `h3to133`: Friend suicide attempt \
    2. `h3to135`: Family suicide attempt \
    3. `h3to134`: Friend suicide completion \
    4. `h3to136`: Family suicide completion \
    \   
  **Wave 4**:\
    1. `h4se4`: Friend or family suicide attempt \
    2. `h4se5`: Family or friend suicide completion \
    \
  **Wave 5**:\
    1. `h5mn11`: Friend or family suicide attempt  \
    2. `h5mn12`: Family or friend suicide completion \
\
...
# peer behavior variables as listed above
peer_list <- c(
              'h1su4', 'h1su6', 'h1su5', 'h1su7', #wave 1
              'h3to133', 'h3to135', 'h3to134', 'h3to136', #wave 3
              'h4se4', 'h4se5', #wave 4
              'h5mn11', 'h5mn12') # Wave 5
...
final_data_all <- data_combined_all %>%
  dplyr::select(aid, tidyselect::all_of(peer_list))
```
...
From `plot_missing()` we see that wave 3 has highest missingness followed by 4, 5, and 1.
...
From the barplot we see that about 1500 people have 4 variables missing (wave 3 has 4 variables) and about 1000 people with 2 variables missing (wave 5 and wave 4 each has 2 variables). Besides, about 9000 have no missing values which means that they participated into all the wave1, wave3, wave4, and wave5. Using `drop_na()` will result in too much missing information without explanation for why those people are missing those variables.
...
```
```{r correlation plot among variables with high missingness, eval=TRUE, cache=TRUE, fig.width=15,fig.height=12}
...
q1_pb <- hist_by_var(final_data_all, c('h1su4', 'h3to133'),2, "Friend suicide attempt")
q2_pb <- hist_by_var(final_data_all, c('h1su6', 'h3to135'), 2,"Family suicide attempt")
q3_pb <- hist_by_var(final_data_all, c('h1su5', 'h3to134'), 2, "Friend suicide completion")
q4_pb <- hist_by_var(final_data_all, c('h1su7', 'h3to136'), 2, "Family suicide completion")
q5_pb <- hist_by_var(final_data_all, c('h4se4', 'h5mn11'), 2, "Friend or family suicide attempt")
q6_pb <- hist_by_var(final_data_all, c('h4se5', 'h5mn12'), 2, "Friend or family suicide completion")

grid.arrange(q1_pb, q2_pb, q3_pb, q4_pb, q5_pb, q6_pb, nrow = 3)
 
```


Here is the visualization of how answers change for the same question over different waves. The distribution for sucicide remains fairly consistent. The distribution for suicide completion seems to change over time to be more even between the two categories in the later wave.
...
peer_and_outcomes <- c(peer_list, "h5ss0b", "h4pe6", "h4pe7") #depression, anxiety, optimism

final_data_corr <- data_combined_all %>%
  dplyr::select(aid, tidyselect::all_of(peer_and_outcomes))
