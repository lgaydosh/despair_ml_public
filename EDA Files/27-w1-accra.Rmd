...
The purpose of this document is to perform EDA on the Wave 1 commerce research variables (2 used).
...
#contextual data
context_file <- str_c("Z:/Gaydosh/ONE - Obesity and Neighborhood Environment Files/w1_ONE/Wave I ACCRA Data/w1accra.xpt")
...
final_data_all <- data_combined_all %>%
  dplyr::select(aid, tidyselect::all_of(w1accra_list))
```
...
#WHY ONLY 3 VARS
w1accra_and_outcomes <- c(w1accra_list, "h5ss0b", "h4pe6", "h4pe7") #depression, anxiety, optimism

final_data_corr <- data_combined_all %>%
  dplyr::select(aid, tidyselect::all_of(w1accra_and_outcomes))
