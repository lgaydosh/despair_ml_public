...
#contextual data
context_file <- str_c("Z:/Gaydosh/ONE - Obesity and Neighborhood Environment Files/w1_ONE/Wave I School Distance Measures/w1schdis.xpt")
...
final_data_all <- data_combined_all %>%
  dplyr::select(aid, tidyselect::all_of(w1schd_list))
```
...
#q1_w1schd <- hist_by_var(final_data_all, c('bst90592', 'bst90593',
...
```{r correlation funnel for variables most of interest, warning=FALSE, cache=TRUE}
 ...
 w1schd_and_outcomes <- c(w1schd_list,
 "h5ss0b", "h4pe6", "h4pe7") #depression, anxiety, optimism

final_data_corr <- data_combined_all %>%
  dplyr::select(aid, tidyselect::all_of(w1schd_and_outcomes))
