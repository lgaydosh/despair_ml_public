...
The purpose of this document is to perform EDA on the childbearing variables (25 used).
The childbearing variables are: \ 


  **Wave 3**:\
    1. `h3mac10`: child protective services \
    \   
  **Wave 4**:\
    1. `h4tr9`: number of pregnancies \
    2. `h4tr10`: number of live births \
    \
  **Wave 5**:\
    1. `h5se21`: number of pregnancies  \
    2. `h5se22`: number of live births \
    3. `h5se23`: number of still living children \
    \
...
#childbearing variables as listed above
ch_list <- c(
              'h3mac10', #wave 3
              'h4tr9', 'h4tr10', #wave 4
              'h5se21', 'h5se22', 'h5se23') #Wave 5
...
final_data_all <- data_combined_all %>%
  dplyr::select(aid, tidyselect::all_of(ch_list))
```
...
q1_ch <- hist_by_var(final_data_all, c('h4tr9', 'h5se21'), 15, "number of pregnancies")
q2_ch <- hist_by_var(final_data_all, c('h4tr10','h5se22'), 12,"number of live births")


grid.arrange(q1_ch, q2_ch, nrow = 1)
 
```

Here is the visualization of how answers change for the same question over different waves. People in wave 5 tended to have more pregnancy and live births than in wave 4, which is logical as time progresses.
...
ch_and_outcomes <- c(ch_list, "h5ss0b", "h4pe6", "h4pe7") #depression, anxiety, optimism

final_data_corr <- data_combined_all %>%
  dplyr::select(aid, tidyselect::all_of(ch_and_outcomes))
