...
The purpose of this document is to perform EDA on the Wave 1 rural/urban variables (1 used).

These variables are: \ 
'w1ru1' : RUCA Code
'w1ru2' : RUCA Metropolitan status code
...
#contextual data
context_file <- str_c("Z:/Gaydosh/ONE - Obesity and Neighborhood Environment Files/w1_ONE/Wave I Rural-Urban Commuting Area (RUCA) Codes/w1ruca.xpt")
...
final_data_all <- data_combined_all %>%
  dplyr::select(aid, tidyselect::all_of(w1ruca_list))
```
...
```
```{r correlation plot among variables with high missingness, eval=FALSE, cache=TRUE, fig.width=15,fig.height=15}
...
#q1_w1ruca <- hist_by_var(final_data_all, c('bst90592', 'bst90593',
...
```{r correlation funnel for variables most of interest, warning=FALSE, cache=TRUE}
 ...
 w1ruca_and_outcomes <- c('w1ru2',
 "h5ss0b", "h4pe6", "h4pe7") #depression, anxiety, optimism

final_data_corr <- data_combined_all %>%
  dplyr::select(aid, tidyselect::all_of(w1ruca_and_outcomes))
