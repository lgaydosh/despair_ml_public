...
The purpose of this document is to perform EDA on the SES future prospects variables (25 used).
The SES future prospects variables are: \ 
'h3ec59', 'h3ec60', 'h4ec19','h5ec9'
...
final_data_all <- data_combined_all %>%
  dplyr::select(aid, tidyselect::all_of(sf_list))
```
...
From `plot_missing()` we see that wave 3 has the most missingness, followed by 4 and then 5.
...
# Explore change over time- none to evaluate
...
sf_and_outcomes <- c(sf_list, "h5ss0b", "h4pe6", "h4pe7") #depression, anxiety, optimism

final_data_corr <- data_combined_all %>%
  dplyr::select(aid, tidyselect::all_of(sf_and_outcomes))
