...
generate_datasets <- function(outcome_var, binarize_outcome,
                              filebase = '/scratch/p_gaydosh_lab', seed_val=9384){
  ...
  ds_raw_outcome <- full_dataset %>%
    ...
    dplyr::select(aid, outcome, all_of(c(predictor_list, demographic_age_list, demographic_list, constructed_outcomes))) %>%
