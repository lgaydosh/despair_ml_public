...
top_ns_group <- exps %>%
  ...
  #dplyr::arrange(desc(!!ensym(metric)), .by_group=TRUE) %>%
  slice(1:n) %>%
...
```
The results above are a result of choosing some metric, and then looking at the top 5 training and top 5 testing models according to that metric, and taking an average of those 5 for sensitivity, specificity, ppv, npv, and mpce.  The training data is shown for general purpose.  Using this code should allow us to understand what the best metric should be, and which method that we should pick to meet that metric.  For example, using mpce as a metric indicates that we'll have pretty good performance if we balance the classes, but this also shows that we will decrease our specificity and ppv a bit.

Running this again using `pr_auc`, for example, we can see similar behavior.  Balancing the classes has the effect of increasing the sensitivity and lowering the mpce, but results in worse positive predictive value.  This basically means that balancing the classes increases false positives.  If we don't care about this, and we truly only care about sensitivity, then we should choose to balance the classes.

This also suggests that we should perhaps use a more palpable metric if possible.  An example would be number of lives saved (impossible from this dataset, although we might be able to pull in others to find some sort of ratio) or similar.
