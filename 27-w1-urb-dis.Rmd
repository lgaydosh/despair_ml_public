...
The purpose of this document is to perform EDA on the Wave 1 urban distance variables (2 used).

These variables are: \ 

'w1ud1990' : 1990 Urbanized Area distance in meters \
'w1ud2000' : 2000 Urbanized Area distance in meters \
...
#contextual data
context_file <- str_c("Z:/Gaydosh/ONE - Obesity and Neighborhood Environment Files/w1_ONE/Wave I Urban Distances/w1urbdst.xpt")
...
w1urd_list <- c('w1ud1990', 'w1ud2000')
...
final_data_all <- data_combined_all %>%
  dplyr::select(aid, tidyselect::all_of(w1urd_list))
```
...
```
```{r correlation plot among variables with high missingness, cache=TRUE, fig.width=15,fig.height=15}
...
#q1_w1urd <- hist_by_var(final_data_all, c('bst90592', 'bst90593',
...
```{r correlation funnel for variables most of interest, warning=FALSE, cache=TRUE}
 ...
 w1urd_and_outcomes <- c(w1urd_list,
 "h5ss0b", "h4pe6", "h4pe7") #depression, anxiety, optimism

final_data_corr <- data_combined_all %>%
  dplyr::select(aid, tidyselect::all_of(w1urd_and_outcomes))
