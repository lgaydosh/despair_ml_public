...
```{r}
xval_save_helper <- function(in_data, xseed, tts_seed, strat_var, no_folds, out_var){
  ...
  #create save variable
  save_csv <- str_c(prefix, out_var, '/kfold_assign_', xseed, '_tts_', tts_seed, '.csv')
  write_csv(xfolds, save_csv)
...
working_ds <- full_dataset %>%
  ...
  dplyr::select(aid, all_of(outcome)) %>%
...
Split the data according to some seed; note: you'll have to use the same seed
```{r}
set.seed(seed)
