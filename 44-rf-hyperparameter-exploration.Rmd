...
max_depths_tune <- c(25,30)
ntrees_tune <- c(100,150)
min_rows_tune <- c(25,40,50,60)
mtries_tune <- c(-1,5,10)
...
Row at the maximum training precision-recall AUC
```{r}
performances %>% 
  filter(training_pr_auc == max(training_pr_auc))
```
...
Row at the maximum testing precision-recall AUC
```{r}
performances %>% 
  filter(testing_pr_auc == max(testing_pr_auc))
```
Based on experimentation with different spans for the hyperparameters, it seems that the following spans should be used when expermenting with creating random forest models. More on this as more experiments are conduncted to confirm or validate these. 
...
p1 <- performances %>% 
  ...
  theme_bw()+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"), panel.border = element_blank())+labs(y = "pr_auc")
