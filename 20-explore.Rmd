...
```{r graph cramers v}
# Plot it
plot_cramer_v <- function(df, value_cutoff = 0, 
                          x_axis_angle = 90, 
 ...
 out <-  df %>% 
    filter(cramer_v >= value_cutoff) %>% 
    ggplot() +
    geom_raster(aes(x = var1, y = var2, fill = cramer_v)) +
...
graph_correlation <- function(df, 
  ...
  # x_angle: angle to tilt x-axis labels
  ...
  df %>% 
    plot_correlation(type = var_type,
                     ...
                     title = plot_title,
                     theme_config = list(plot.title = element_text(hjust = 0.5, size = title_size),
                               axis.text.x = element_text(angle = x_angle, size = x_axis_size),
                               axis.text.y = element_text(size = y_axis_size)))
}
