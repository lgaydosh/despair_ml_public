...
# this function takes a look at the outcomes of interest and returns a table 
unique_outcomes <- function(df, outcome, is_factor = F){
  # df : working dataframe
  # outcome : outcome to make predictions on - sent in as a string
  df <- df %>% 
    dplyr::select(outcome) %>% 
    table(useNA="always") %>% 
    data.frame() %>% 
    rename("Outcome" = ".") %>% 
    mutate(Type = if_else(is_factor == T, levels(Outcome) %>% typeof(), Outcome %>% typeof()))
...
# Example 
# require(tidyverse)
# x <- iris %>% unique_outcomes("Species", is_factor = T)
# x
...
explore_outcome <- function(df, column, binary = TRUE, legit_skip = FALSE, skip_var = 99){
  # df : working dataframe
  ...
  if (binary == TRUE){
    df %>% 
      ...
      summarize(count = n()) %>% 
      ungroup()  %>% 
      ggplot(aes_string(x = column, y = "count"))+
      geom_col() +
      theme_minimal() +
      labs(title="Count of Outcome",
           x = column, y = paste0("Count of ", column))
  }
    ...
    if(legit_skip == FALSE){
      ...
      plot2 <- df %>% 
        ...
        geom_col() +
        theme_minimal() +
        labs(title="Count of Outcome(Binarized)",
             x = column, y = paste0("Count of ", column))
      ...
      plot2 <- df %>% 
        binarize_var(column, legit_skip = legit_skip, skip_var = skip_var) %>% 
        dplyr::select(column) %>% 
        group_by_(column) %>% 
        summarize(count = n()) %>% 
        ungroup()  %>% 
