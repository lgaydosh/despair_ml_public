...
add_hv_drink <- function(df){
  ...
  df %>%  
    ...
    mutate(hv_drink = case_when(
      #sex at birth
      ((weekly_drinks>15) & (h5od2a==1))|((weekly_drinks>8)&(h5od2a==0)) ~ 1,
...
#Add binge drinking
```{r}
#combine all drinking variables to calculate days of binge drinking in past year
#female 4+ drinks, male 5+ drinks
add_binge_drink <- function(df){
  df %>% 
    mutate(binge_drink = case_when(
...
```{r}
constructed_outcomes = c('p_drug', 'i_drug', 'hv_drink', 'binge_drink')
```
