...
pacman::p_load(SASxport, haven, dplyr, tidyr, janitor, naniar, ggplot2, Hmisc, purrr, scales, reshape, HH, tibble, likert, reshape2,forcats)
```
...
outer_joined_waves <-  wave1 %>%
    #inner_join(wave2, by = "AID") %>%
    full_join(wave3, by = "AID") %>%
    full_join(wave4, by = "AID") %>%
    full_join(wave5, by = "AID") %>% 
  clean_names() %>% 
  mutate_at(vars(starts_with("h")), as.factor) # Subject to change, this is under the assumption that essentially all the variables start with 'h'
```
...
predictor_vars <- outer_joined_waves[which(colnames(outer_joined_waves) %in% predictor_list)] %>% 
  mutate_at(vars(starts_with("h")), as.factor)
...
```{r}
outcome_vars <- outer_joined_waves[which(colnames(outer_joined_waves) %in% outcome_vars_list)] %>% 
  mutate_at(vars(starts_with("h")), as.factor)
