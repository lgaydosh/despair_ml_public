...
add_wave_5_anthro <- function(df, filepath = "~/gvfs/smb-share:server=vu1file.it.vanderbilt.edu,share=gaydosh_addhealth/Gaydosh/Wave V Biomarker Files/Wave V Anthropometrics/banthro5.xpt"){
  ...
  df <- df %>% inner_join(anthro, by = "aid") %>% select(aid, h5bmi, h5waist, everything())
...
add_wave_3_bmi <- function(df, weight_lb, height_ft, height_in ){
  ...
  df['height_m'] <- (df[[height_ft]]*12 + df[[height_in]])/39.37
  df['h3bmi'] <- df[['weight_kg']]/(df[['height_m']] * df[['height_m']])
