...
add_demographics <- function(df){
  ...
  df %>% 
    ...
    mutate(one_race5 = case_when( #one race: first analyze those with multiple races
                        ...
                        (h5od8==997) ~ 1*h5od4a +2*h5od4b +3*h5od5c +4*h5od5d +5*sum(h5ode,h5odf,h5odg), 
                        TRUE ~ NA))
}
```

The following function is used because for some reason, there are some subjects in Waves 2 and 3 that are not in Wave 1.  The following function loads the errant AIDs from a remote server and then removes them from the desired dataset.
