...
add_wave_4_glucose <- function(df, filepath = "~/gvfs/smb-share:server=vu1file.it.vanderbilt.edu,share=gaydosh_addhealth/Gaydosh/Wave IV Biomarker Files/Wave IV Glucose/glu_a1c.xpt"){
  ...
  glucose <- glucose %>% clean_names() %>% select(aid, hba1c)
...
add_wave_4_crp <- function(df, filepath = "~/gvfs/smb-share:server=vu1file.it.vanderbilt.edu,share=gaydosh_addhealth/Gaydosh/Wave IV Biomarker Files/Wave IV CRP-EBV/crp_ebv.xpt" ){
  ...
  crp_df <- crp_df %>% clean_names() %>% select(aid, crp)
...
add_wave_4_lipids <- function(df, filepath = "~/gvfs/smb-share:server=vu1file.it.vanderbilt.edu,share=gaydosh_addhealth/Gaydosh/Wave IV Biomarker Files/Wave IV Lipids/lipids.xpt" ){
  ...
  lipids <- lipids %>% clean_names() %>% select(aid, hdl, ldl, tg)
...
add_wave_3_bmi <- function(df, weight_lb, height_ft, height_in ){
  # df - full dataframe with the weight and height variables present
  # weight_lb - weight(in lbs) variable name passed as string. Converted to kgs by dividing by 2.2
  # height_ft, height_in - height variable names passed as string. 
  ### Converted to ms by:
  ...
  df['weight_kg'] <- df[[weight_lb]]/2.2 # lbs -> kgs
  df['height_m'] <- (df[[height_ft]]*12 + df[[height_in]])/39.37 # inches -> meters
...
# Example
# full_dataset %>% 
#   add_wave_3_bmi("h3wgt", "h3hgt_f", "h3hgt_i")
```
