...
add_demographics <- function(df){
  ...
  df %>% 
    mutate(birth_in_months = (as.double(str_c("19",h1gi1y))*12 )+ as.double(h1gi1m)) %>% 
    mutate(age_w1 = floor((((as.double(str_c("19",iyear))*12 )+imonth) - birth_in_months)/12) ) %>% 
...
# Example:
# working_ds
# working_ds <- working_ds %>% binarize_var('times_used_x')
