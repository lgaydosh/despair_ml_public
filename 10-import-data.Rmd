...
recode_missing_factors<-function(df, na_levels){
  ...
  for (i in 1:length(vars_with_miss)){
    ...
    mlevels <- na_levels %>% 
      filter(variable_name == vars_with_miss[i]) %>% 
      select(na_values) %>% 
      pull() %>% 
      str_split(',') %>% 
      .[[1]] %>% 
...
```{r}
load_predictors <- function(df, sheet_num){
  # Function to load predictor variables from different sheets in the variables_of_interest file
  ## we will use this function to load different types of predictors: despair predictors, relevant individual predictors, contextual predictors
  # df: dataframe of wave data
  # sheet_num : this sheet number
  
  sheet_df <- read_excel("Variables_of_Interest_9.xlsx", sheet = sheet_num) %>% clean_num()
  df <- recode_missing_factors(df, sheet_df)
  df <- recode_missing_numeric(df, sheet_df)
  return(df)
}
```
