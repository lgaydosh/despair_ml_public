...
add_demographics <- function(df){
  ...
  df %>% 
    dplyr::mutate(birth_in_months = (as.double(str_c("19",h1gi1y))*12 )+ as.double(h1gi1m)) %>% 
    dplyr::mutate(age_w1 = floor((((as.double(str_c("19",iyear))*12 )+imonth) - birth_in_months)/12) ) %>% 
    dplyr::mutate(age_w3 = floor((((iyear3*12)+imonth3)-birth_in_months)/12)) %>% 
    dplyr::mutate(age_w4 = floor((((iyear4*12)+imonth4)-birth_in_months)/12)) %>% 
    dplyr::mutate(age_w5 = floor((((iyear5*12)+imonth5)-birth_in_months)/12)) %>% 
