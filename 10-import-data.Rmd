...
recode_missing_factors<-function(df, na_levels){
  ...
  for (i in 1:length(var_names)){
    ...
    if(!is.na(mlevels[1])){ 
      df[[var_names[i]]] <-ifelse(df[[var_names[i]]] %in% mlevels, NA, df[[var_names[i]]])
    }
...
```{r}
recode_variables <- function(df, sheet_num, outcome = NULL, binary = T){
  ...
  # binary : if F means that the outcome variable needs to be binarized
