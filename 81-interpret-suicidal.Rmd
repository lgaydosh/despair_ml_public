...
results_directory <- str_c(fbase, outcome, '/results_test_run')
set.seed(seed)
```
...
```{r warning=FALSE, message=FALSE}
ds_raw %>% 
  binarize_var(outcome, legit_skip = TRUE, skip_var = NULL) %>% 
  group_by(.data[[outcome]]) %>% 
  summarise(total = n(), type = class(.data[[outcome]]))

ds_raw %>% 
  explore_outcome(outcome, binary = TRUE, legit_skip = FALSE, skip_var = NULL)
```
...
# get data files
bs_rf_perf = load_from_csv(bs_rf_perf, results_directory, no_tasks)
...
# get data files
bs_rf_mdi = load_from_csv(bs_rf_mdi, results_directory, no_tasks)
...
# get data files
bs_lasso_perf = load_from_csv(bs_lasso_perf, results_directory, no_tasks)
...
# get data files
bs_lasso_mdi = load_from_csv(bs_lasso_mdi, results_directory, no_tasks)
...
```{r bs feature importance plotly, fig.width = 16, fig.height = 14}
