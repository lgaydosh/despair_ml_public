...
suicide_df <- joined_waves %>%
  get_outcomes_df(outcome = suicide_w5, 
                  predictors = setdiff(predictor_list, c("aid", "age_w4","age_w3","age_w1")))
...
suicide_out_lasso <- model_feature_selection("Lasso",training_frame = training_frame,
                      validation_frame = validation_frame,
...
In this section, I explore how to extract the label names from the variables based on the dataframe attributes
```{r}
#get a single wave
wave1 <- load_waves(c(1))
wave1 <- wave1[[1]]

#we can get the attributes like so
attributes(wave1$aid)$label

#make an example field and determine how to extract everything before the .
test_var <- c('h1fs1.3')
res <- str_split(test_var, '[.]', simplify=TRUE); res
attributes(wave1[[res[1,1]]])

```
The purpose of this function is to get the attributes based on the one-hot encoded variables + factor levels.  It is replicated and more well commented in 50.
```{r}
...
# Aggregating relevant one-hot encoded variables into a single variable (e.g., h5ss0b.1, h5ss0b.2 into h5ss0b)

```{r}
# start with something like mdi from models
