...
pacman::p_load(SASxport, haven, gtools, tidyverse, janitor, naniar, Hmisc, scales, reshape, HH, likert, reshape2,forcats,splitstackshape,rsample, h2o, furrr, rcompanion, ggcorrplot, plotly)
...
knitr::include_graphics('./img/pipeline_overview.png')
```
# Dataset generation
...
suicide_ds <- full_dataset %>%
  ...
  remove_subjects_not_in_wave1(filebase='G:') %>%
...
boot_rf_perm_plt <- suicide_rf$models %>%
  get_aggregated_permute_imp(training_df, outcome='h5mn8')
```
...
With the final models generated, we're now able to compare their performance metrics.  The following table shows the comparison between models in terms of the validation set.  We can select our final model based on the best performing model according to the metric.
...
testing_perf <- get_metric_set_from_models(testing_df, list(final_model_rf[[1]], final_model_lasso[[1]])) %>%
  mutate(model = c('rf', 'lasso'))

print(valid_perf)
```

This is the performance of both the models.  Note that although we don't use this test set to evaluate the final models, we can still see how our selected method would have performed.
```{r}
print(testing_perf)
```
...
metrics_of_interest = c('model', 'accuracy', 'bal_accuracy', 'mpce', 'sens', 'spec', 'ppv', 'npv')
valid_plt <- plot_metric_set(dplyr::select(valid_perf, metrics_of_interest), plot_title = "Model comparison for validation set")
test_plt <- plot_metric_set(dplyr::select(testing_perf, metrics_of_interest), plot_title = "Model comparison for testing set")
gridExtra::grid.arrange(valid_plt, test_plt)
```
