...
cbind(boot_rf_mdi[1:20,], select(boot_rf_perm[1:20,], -met))
```
```{r fig.width = 10, fig.height = 12}
...
Now, let's compare these feature importances based on mdi vs permutation importance:
```{r fig.width = 16, fig.height = 14}
...
## Predictor comparison based on model type
...
# # Spans of hyper parameters for random forest
rf_params <- list(max_depth = seq(10, 50, by=10),
                  ntrees = 150,
                  mtries = seq(-1, 30, by=5),
                  min_rows = seq(10, 30, by=5),
                  balance_classes = c(TRUE, FALSE),
                  stopping_metric = c('mean_per_class_error', 'AUCPR', 'misclassification'),
                  categorical_encoding = 'one_hot_explicit')
...
final_model_rf <- rf_model(outcome,
                           ...
                           validation_frame = validation_df,
                           nfolds = 5,
                           hyper_params = rf_params, seed=seed)
