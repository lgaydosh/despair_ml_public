...
```{r}
top_lasso <- top_preds_outcome_lasso %>% 
  mutate(var_name = sub("\\..*", "", variable)) %>% # get the name of the variable before 
  .$var_name

top_rf <- top_preds_outcome_rf %>% 
  .$variable

top_model_preds <- c(top_lasso, top_rf)

top_model_preds %>% table()
```
