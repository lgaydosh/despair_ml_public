...
```{r include=FALSE}
# The following code is used to adapt the use of this notebook with SLURM functionality.  If we arrived at this notebook (actually, the associated R script) via SLURM, we'll have a variable set called `from_script`.  If we DON'T have this variable, that means we're probably trying to run it from portal.  Otherwise, let's gather the command line parameters.

make_run_settings <- function(){
  
  #settings for parameters in the global environment
  results_directory <<- '/scratch/p_gaydosh_lab'
  no_tasks <<- 2
}

tryCatch({from_script},
         error = function(e){
           from_script <<- FALSE
         })
...
```{r initializations, include=FALSE, eval=from_script}
h2o.init() 
h2o.no_progress()
future::plan(multiprocess)
```

```{r seeds for reproducibility, eval=from_script}
#seed= 3895
set.seed(seed)
```

# Pipeline overview
```{r echo=FALSE, out.width='50%', eval=!from_script}
...
```{r warning=FALSE, message=FALSE, eval=!from_script}
suicide_ds %>% 
...
```{r binarize and drop NA outcome, warning=FALSE, message=FALSE, eval=!from_script}
...
```{r eda distributions, fig.width = 15, fig.height=15, eval=!from_script}
...
```{r eda missing, eval=!from_script}
suicide_ds %>%
...
```{r eda correlations, eval=!from_script}
suicide_ds %>%
  #dplyr::select(1:20) %>%
  pairwise_cramers_v() %>%
...
```{r, message=FALSE, warning=FALSE, eval=from_script}
...
```{r feature selection rf, include=FALSE, eval=from_script}
...
if (from_script) {
  ...
  bs_rf_files <- str_c(results_directory, str_c('/bs_rf_perfs_', seq(1,no_tasks)), '.csv')
...
```{r evaluate bootstrap model performance rf, warning=FALSE, message=FALSE, eval=!from_script}
...
if (from_script) {
  ...
  # map read them into a df
  bs_rf_mdi <- map_dfr(bs_rf_mdi_files, read_csv)
}

```

```{r evaluate bootstrap mdi, eval=!from_script}
...
```{r fig.width = 10, fig.height = 12, eval=!from_script}
...
```{r aggregate rf perm results, eval=!from_script}
met <- 'pr_auc'
...
```{r fig.width = 16, fig.height = 14, eval=!from_script}
...
```{r feature selection lasso, message=FALSE, warning=FALSE, eval=from_script}
# Function parameters
...
if (from_script) {
  ...
  # get array of files to read
  bs_lasso_files <- str_c(results_directory, str_c('/bs_lasso_perfs_', seq(1,no_tasks)), '.csv')
...
```{r evaluate bootstrap model performance lasso, warning=FALSE, message=FALSE, eval=!from_script}
...
if (from_script) {
  ...
  # map read them into a df
  bs_lasso_mdi <- map_dfr(bs_lasso_mdi_files, read_csv)
}

```

```{r show bs lasso normalized coefficients, eval=!from_script}
...
if (from_script) {
  ...
  write_csv(bs_lasso_perm_plt, bs_lasso_perm_file)
...
```{r aggregate lasso permutations and get metrics, eval=!from_script}
...
```{r plot lasso permutation, fig.width = 10, fig.height = 12, eval=!from_script}
...
```{r compare bs mean performance, eval=!from_script}
...
```{r compre bs feature importance, eval=!from_script}
...
```{r final model evaluation rf, eval=from_script}
...
if (from_script) {
  ...
  # get array of files to read
  final_rf_files <- str_c(results_directory, str_c('/final_rf_perfs_', seq(1,no_tasks)), '.csv')
  
  # map read them into a df
  final_rf_perfs <- map_dfr(final_rf_files, read_csv)
  
  # get the index of the best performance
  final_rfmodel_ind <- which.max(dplyr::select(final_rf_perfs, met))
  
  final_rfmodel_path <- str_c(results_directory, str_c('/final_rf_model_', final_rfmodel_ind))
  final_rfmodel <- h2o.loadModel(final_rfmodel_path)
...
```{r rf final model performance, eval=!from_script}
...
if (from_script) {
  final_rf_perm_plt <- final_model_rf[[1]] %>%
    get_aggregated_permute_imp(training_df, outcome=outcome)
...
```{r get metrics for final rf, eval=!from_script}
...
```{r plot rf final permutation, fig.width = 10, fig.height = 12, eval=!from_script}
...
```{r compare rf final with bootstrap, eval=!from_script}
...
```{r plot final rf features plotly, fig.height=14, fig.width=16, eval=!from_script}
# Comparison of top_n features
rf_joined_results %>%
...
```{r final model evaluation lasso, message=FALSE, warning=FALSE, eval=from_script}
# Function parameters
lasso_params <- list(alpha = c(1))
...
if (from_script) {
  ...
  # get array of files to read
  final_lasso_files <- str_c(results_directory, str_c('/final_lasso_perfs_', seq(1,no_tasks)), '.csv')
  
  # map read them into a df
  final_lasso_perfs <- map_dfr(final_lasso_files, read_csv)
  
  # get the index of the best performance
  final_lassomodel_ind <- which.max(dplyr::select(final_lasso_perfs, met))
  
  final_lassomodel_path <- str_c(results_directory, str_c('/final_lasso_model_', final_lassomodel_ind))
  final_lassomodel <- h2o.loadModel(final_lassomodel_path)
...
```{r lasso final model performance, eval=!from_script}
# show model final performance
...
if (from_script) {
  final_lasso_perm_plt <- final_model_lasso[[1]] %>%
    get_aggregated_permute_imp(training_df, outcome=outcome)
...
```{r get permute features for final lasso, eval=!from_script}
...
```{r plot lasso final permutation, fig.width = 10, fig.height = 12, eval=!from_script}
...
```{r compare lasso final with bootstrap, eval=!from_script}
...
```{r plot lasso feature comparison, fig.width = 16, fig.height = 14, eval=!from_script}
# Comparison of top_n features
...
```{r compare lasso vs random forest features final, eval=!from_script}
...
```{r plotly lasso v random forest features, eval=!from_script}
# Comparison of top_n features
