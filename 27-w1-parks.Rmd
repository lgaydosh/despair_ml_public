...
The purpose of this document is to perform EDA on the Wave 1 park variables (1 used).

These variables are: \ 

'w1pkc1k1' : Park count, 1km
...
#contextual data
context_file <- str_c("Z:/Gaydosh/ONE - Obesity and Neighborhood Environment Files/w1_ONE/Wave I Parks Data/w1parks.xpt")
...
final_data_all <- data_combined_all %>%
  dplyr::select(aid, tidyselect::all_of(w1pk_list))
```
...
#q1_w1pk <- hist_by_var(final_data_all, c('bst90592', 'bst90593',
...
```{r correlation funnel for variables most of interest, warning=FALSE, cache=TRUE}
 ...
 w1pk_and_outcomes <- c(w1pk_list,
 "h5ss0b", "h4pe6", "h4pe7") #depression, anxiety, optimism

final_data_corr <- data_combined_all %>%
  dplyr::select(aid, tidyselect::all_of(w1pk_and_outcomes))
