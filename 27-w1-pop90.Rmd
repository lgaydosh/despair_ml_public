...
The purpose of this document is to perform EDA on the Wave 1 population density variables (2 used).

These variables are: \ 

'w1p90_a1' : block group area (sq m), 1 km \

'w1p90_p1' : block group population, 1 km \
...
#contextual data
context_file <- str_c("Z:/Gaydosh/ONE - Obesity and Neighborhood Environment Files/w1_ONE/Wave I Population Density Data/w1pop90.xpt")
...
w1pop_list <- c('w1p90_a1', 'w1p90_p1')
...
final_data_all <- data_combined_all %>%
  dplyr::select(aid, tidyselect::all_of(w1pop_list))
```
...
#q1_w1pop <- hist_by_var(final_data_all, c('bst90592', 'bst90593',
...
```{r correlation funnel for variables most of interest, warning=FALSE, cache=TRUE}
 ...
 w1pop_and_outcomes <- c(w1pop_list,
 "h5ss0b", "h4pe6", "h4pe7") #depression, anxiety, optimism

final_data_corr <- data_combined_all %>%
  dplyr::select(aid, tidyselect::all_of(w1pop_and_outcomes))
