...
```{r}
full_dataset %>% 
  unique_outcomes("h5to21", is_factor = F)
```
...
marijuana_ds <- full_dataset %>%
  ...
  binarize_var("h5to21", legit_skip = T, skip_var = 97)  # legitimate skipped variable = 97
```
...
marijuana_ds %>% 
  unique_outcomes("h5to21", is_factor = F)
```
There are 11 NAs introduced into the outcome after filtering the aids to only subjects that were in all 5 waves. 

```{r}
marijuana_ds <- marijuana_ds %>% drop_na(outcome)
