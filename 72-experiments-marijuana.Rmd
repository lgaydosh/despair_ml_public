...
## top feature outcome
n=2
top_preds_outcome_rf <- marijuana_out[[3]] %>%
  get_total_placement() %>% 
...
#%>%
#    group_by(variable) %>% 
#    summarise(median = median(placement, na.rm = TRUE)) ##?????????? is this what we want? then no need for boxplot
...
top_lasso <- top_preds_outcome_lasso %>% 
  dplyr::select(variable,avg_place)%>%
  arrange(desc(avg_place))%>%
  drop_na()
...
stringdist_left_join(top_lasso, top_rf,
     by ="variable", distance_col = NULL)
```
