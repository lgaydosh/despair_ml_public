...
This is the table with all the counts for different levels of the variables. We can see 97 is a legitimate skip variable. We can send this into the binarize_var function and it will be coded as 0 (did not use marijuana). 

```{r}
full_dataset %>% 
  group_by(h5to21) %>% 
  summarise(total = n(), type = class(h5to21))
```


This is what the dataset looks like if the variables were binarized. We see 8488 `NA`values but these will be dropped after only the subjects that were present in all waves are kept.
```{r}
full_dataset %>% 
  binarize_var("h5to21", legit_skip = T, skip_var = 97) %>% 
  group_by(h5to21) %>% 
  summarise(total = n(), type = class(h5to21))
```
...
full_dataset %>% 
  explore_outcome("h5to21", binary = F, legit_skip = T, skip_var = 97)
```
```{r}
...
marijuana_ds <- full_dataset %>%
  ...
  fix_outcome_variable("h5to21", is_binary = FALSE, is_skip = TRUE, miss_var = 97)
...
```{r}
marijuana_ds %>% explore_outcome("h5to21")
```
This is the counts of the outcome variable for the 
...
```{r eda fig.width = 15, fig.height=15}
...
full_dataset %>%
  ...
  filter(aid %in% inner_aids) %>%
  remove_subjects_not_in_wave1() %>%
  recode_missing_factors(na_levels) %>% 
  recode_missing_numeric(na_levels) %>% 
  binarize_var("h5to21", legit_skip = T, skip_var = 97)  
...
full_dataset %>%
  ...
  remove_subjects_not_in_wave1() %>%
  #fix_outcome_variable("h5to21", is_binary = FALSE, is_skip = TRUE, miss_var = 97)%>% 
  binarize_var("h5to21", legit_skip = TRUE, skip_var = 97) %>% 
...
- Here, we will justify the feature selection based o
n performance metrics; we assert that because of good model performance, we can expect that the features selected are reasonable because the models fit the data well (without being overtrained)
...
marijuana_rf <- model_feature_selection("RF",training_frame = training_df,
...
mean_bs_rf_perf <- get_metric_set_from_perfs(marijuana_rf$perfs) %>%
...
plot_placement_boxplot(marijuana_rf$mdi)
```

Now, let's look at the permutation importance:
```{r}
boot_rf_perm_plt <- marijuana_rf$models %>%
...
marijuana_lasso <- model_feature_selection( "Lasso",
...
mean_bs_lasso_perf <- get_metric_set_from_perfs(marijuana_lasso$perfs) %>%
  dplyr::select(accuracy, mpce, sens, spec, ppv, npv, roc_auc, pr_auc,
                tns, tps, fns, fps, no_n, no_p,  err_rate, bal_accuracy, everything()) %>%
  summarise_if(is.numeric, mean, na.rm=TRUE) %>% 
...
```{r}
boot_lasso_mdi <- marijuana_lasso$mdi %>%
  get_median_placement(use_base_var = TRUE) %>%
  add_attribute_names('predictor', full_dataset) %>%
  dplyr::select(predictor, att_name, overall_rank)
...
```{r fig.width = 10, fig.height = 12}
plot_placement_boxplot(marijuana_lasso$mdi)
```
#### Permutation importance
```{r}
boot_lasso_perm_plt <- marijuana_lasso$models %>%
  get_aggregated_permute_imp(training_df, outcome=outcome)
```
