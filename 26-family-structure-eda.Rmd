...
The purpose of this document is to perform EDA on the family structure variables (10 used).
The family variables are: \ 
'h3cj160',
'h4wp3', 'h4wp9', 'h4wp16', 'h4wp30',
'h5wp1', 'h5wp15', 'h5wp4', 'h5wp18', 'h5wp34'
...
fs_list <- c(
                  'h3cj160', #wave 3
                  'h4wp3', 'h4wp9', 'h4wp16', 'h4wp30', #wave 4
                  'h5wp1', 'h5wp15', 'h5wp4', 'h5wp18','h5wp34') # Wave 5
...
final_data_all <- data_combined_all %>%
  dplyr::select(aid, tidyselect::all_of(fs_list))
```
...
About 7% missing observations which remind us to be careful when deciding to drop NA.
...
q1_fs <- hist_by_var(final_data_all, c('h3cj160','h4wp30','h5wp15'),1.5, "biological father ever in jail")

grid.arrange(q1_fs, nrow = 1)
 
```


Here is the visualization of how answers change for the same question over different waves. The proportions of the answers appear relatively steady.
...
fs_and_outcomes <- c(fs_list, "h5ss0b", "h4pe6", "h4pe7") #depression, anxiety, optimism

final_data_corr <- data_combined_all %>%
  dplyr::select(aid, tidyselect::all_of(fs_and_outcomes))
