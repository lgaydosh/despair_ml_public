---
title: "98-outcome-feature-graphs2-h5mn8"
...
port_no <- start_h2o()
h2o.no_progress()
future::plan(multicore) #multiprocess
```
...
results_directory <- str_c(fbase, outcome, '/new_data_1')
```
...
```{r}
write.csv(bs_rf_perm_20_variable,"bs_rf_perm_20_variable_h5mn8Dist.csv", row.names = FALSE)
```
...
```{r}
newdata_drop <- droplevels(newdata)
```

```{r}
levels(newdata$predictor)
```

```{r}
newdata_drop <- factor(newdata_drop$predictor, levels = newdata_drop$predictor)

```

```{r}
write.csv(newdata,"data_for_graph_h5mn8Dist.csv", row.names = FALSE)
```



```{r, fig.width=8, fig.height=20}
ggplot(newdata, aes(x = placement, y = predictor,height = stat(count), fill = factor(Category))) +
  geom_density_ridges(data=subset(newdata,ToHighlight == "yes"), fill="#d96473", stat="binline", bins = 20, binwidth = 1, scale = 2, draw_baseline = FALSE) +
  geom_density_ridges(data=subset(newdata,ToHighlight == "no"), stat="binline", bins = 20, binwidth = 1, scale = 2, draw_baseline = FALSE) +
  scale_fill_manual(name = "Domains", values = c("#f2d5d5","#f2edd5","#dff2d5","#d5e2f2","#e9d5f2")) + scale_y_discrete(name ="Top 20 Predictors") + scale_x_discrete(name ="Placements on Prob. drink RF model")+ theme_linedraw()
```




levels = "h5ss0e", "h5id6g", "h5ss0b", "h5ss0a", "h5ss0c", "h5id6i", "h5pe1",  "h5mn2",  "h4mh2", "h5mn4",  "h4id5h", "h5ss0d", "h5mn1",  "h5mn3",  "h4mh19", "h5pe2",  "tg",     "hdl",   "h4mh3",  "h4mh27" 



levels = c( "life_not_worth_living_w5", "dx_depression_w5", "depressed_w5",       "shake_off_blues_w5", "happy_w5", "dx_anxiety_w5", "optimistic_w5",            "handle_problems_w5", "feel_isolated_w4", "cannot_overcome_w5", "dx_depression_w4",         "sad_w5",  "control_w5", "things_going_my_way_w5", "shake_off_blues_w4",       "pessimisitic_w5", "trigylerides_w4", "hdl_w4", "control_w4", "disliked_w4" )  
...
ggplot(lasso_perm_in_domain_for_graph2, aes(x = placement, y = predictor,height = stat(count), fill = factor(Category))) +
  ...
  scale_fill_manual(name = "Domains", values = c("#f2d5d5","#f2edd5","#dff2d5","#d5e2f2","#e9d5f2")) + scale_y_discrete(name ="Top 20 Predictors") + scale_x_discrete(name ="Placements on Suic. idea. LASSO model")+ theme_linedraw()
```
