...
permute_columns <- function(df, outcome){
  ...
  predictor_names <- df %>% dplyr::select(-outcome) %>% names()
...
plot_permute_var_imp <- function(df_permute_var_imp, metric){
  ...
  plot_df %>% 
    ...
    geom_bar(aes(x = reorder(att_name, {{metric}}), 
                 y = {{metric}}), 
             stat = "identity",
             fill = "light blue") +
    coord_flip() +
...
get_aggregated_permute_imp <- function(mdl_list, newdata_df, outcome='h5mn8'){
