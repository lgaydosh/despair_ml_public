...
rf_feature_selection <- function(training_df, n, k = 5, max_depths_tune = c(100,150,200), ntree_tune = c(5,10,25), outcome, top_n){
  ...
  for(i in 1:length(straps$splits)){
    results <- as.data.frame(straps$splits[[i]])%>%
      boot_model(outcome = outcome, max_depths = max_depths_tune, ntrees = ntree_tune)
