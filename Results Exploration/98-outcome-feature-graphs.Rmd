---
title: "98-outcome-feature-graphs"
...
```{r}
write.csv(bs_rf_perm_20_variable,"bs_rf_perm_20_variable_probdrinkDist.csv", row.names = FALSE)
```
...
```{r}
write.csv(newdata,"data_for_graph_probdrinkDist.csv", row.names = FALSE)
```
...
ggplot(lasso_perm_in_domain_for_graph2, aes(x = placement, y = predictor,height = stat(count), fill = factor(Category))) +
  ...
  scale_fill_manual(name = "Domains", values = c("#f2d5d5","#f2edd5","#dff2d5","#d5e2f2","#e9d5f2")) + scale_y_discrete(name ="Top 20 Predictors") + scale_x_discrete(name ="Placements on Prob. drink LASSO model")+ theme_linedraw()
```
